...
void FW::floyd_warshall_foreach_k(int k)
{
	for (int i = 1; i <= mNoOfNodes; i++)
	{
		for (int j = 1; j <= mNoOfNodes; j++)
...
void FW::floyd_warshall_stlparallel()
{
	for_each(execution::par, mKVector.begin(), mKVector.end(), bind(&FW::floyd_warshall_foreach_k, this, placeholders::_1));
}
