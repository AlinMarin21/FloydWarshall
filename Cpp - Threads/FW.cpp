...
void FW::floyd_warshall(int k_row)
{
	mtx.lock();

	for (int k = k_row; k < k_row + (mNoOfNodes / NTHREADS); k++)
	{
		for (int i = 1; i <= mNoOfNodes; i++)
		{
			for (int j = 1; j <= mNoOfNodes; j++)
...
void FW::floyd_warshall_threads()
{
	int k_row = 1;
	for (int i = 1; i <= NTHREADS; i++)
	{
		mThreads.push_back(thread(&FW::floyd_warshall, this, k_row));
		
		k_row += mNoOfNodes / NTHREADS;
	}
